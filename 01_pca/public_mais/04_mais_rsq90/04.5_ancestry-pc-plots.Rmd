...
(svd.obj1$u %>% dim(.))[1] # 4467 subjects
(svd.obj1$v %>% dim(.))[1] # 94976 SNPs
...
df1 <- subject.df %>% 
  filter(.,(grepl(x=subject.vec,pattern="HGDP"))) # 905
df2 <- subject.df %>% 
  filter(.,(grepl(x=subject.vec,pattern="MCPS"))) # 100 
df3 <- subject.df %>% filter(.,!(grepl(x=subject.vec,pattern="HGDP"))) %>% 
  filter(.,grepl(x=subject.vec,pattern="HG")) # 1648 
df4 <- subject.df %>% filter(.,!(grepl(x=subject.vec,pattern="HGDP"))) %>% 
  filter(.,grepl(x=subject.vec,pattern="NA")) # 823
remove.vec <- rbind(df1,df2,df3,df4)$subject.vec
mais.df <- filter(subject.df,!(subject.vec%in%remove.vec)) # 590
...
samples.hgdp %>% length(.) # 905 HGDP samples
samples.1kg %>% length(.) # 2471 1KG samples
samples.mcps %>% length(.) # 500 MCPS samples
```

Projected sample counts
```{r}
projected.count <- dim(pc.df1)[1]-length(subject.vec) # 138193
projected.samples <- pc.df1$sample.ID[!(pc.df1$sample.ID %in% subject.vec)]
projected.samples %>% length(.) # 138193 samples
...
mais.pdf <- filter(proj.df,!(subject.vec%in%remove.vec)) # 126
...
projected.hgdp %>% length(.) # 24 HGDP samples
projected.1kg %>% length(.) # 33 1KG samples
